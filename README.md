...
🚧 **MVP в активной разработке.** Платформа работает, AI-маппинг через OpenRouter дорабатывается. Совместная разработка с экспертом-сооснователем (строительные тендеры).
...
```bash
# 1. Клонировать
git clone https://github.com/alexeymasalykin/tendermargin.git
cd tendermargin
